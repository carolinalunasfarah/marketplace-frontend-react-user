    ...
    <Container fluid className="bg-body-secondary">
      <Row>
        <Col className="col-2 d-flex flex-column justify-content-start align-items-center bg-white pt-4 ps-0 m-3 rounded-4 box-shadow">
          {Auth.user.avatar_url ? <Image src={Auth.user.avatar_url} width={100} className="rounded-circle" /> : <i className="bi bi-camera fs-1 px-4 py-3 bg-body-secondary rounded-circle"></i>}
          <h2 className="fs-5 m-2 text-center">{Auth.user.firstname} {Auth.user.lastname}</h2>
          <p>{Auth.user.email}</p>
          <p>{stars}</p>
        ...
        <Col className="col-7 bg-body-tertiary p-4 m-3 rounded-4 box-shadow">
          {!isLinkClicked ? (
            <div className="d-flex flex-column justify-content-center align-items-center">
              ...
              <p>Este es tu dashboard de <b>{Auth.user.role === "registered" ? "usuario" : "administrador"}</b></p>
              <p>{Auth.user.role === "registered" ?
